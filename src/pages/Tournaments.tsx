...
import { Calendar, Trophy, Users, Clock, Gamepad } from 'lucide-react';
...
const Tournaments = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-candy-light-pink">
      {isLoading ? (
        <div className="h-screen w-full flex items-center justify-center">
          <div className="flex flex-col items-center">
            <h1 className="text-4xl md:text-5xl font-bold text-gradient animate-pulse-soft">
              Tournament<span className="text-candy-bright-pink">Hub</span>
            </h1>
            <p className="mt-4 text-foreground/80 animate-pulse-soft">Loading tournaments...</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <section className="pt-28 pb-16 px-6 min-h-screen bg-candy-light-pink bg-[url('data:image/svg+xml,%3Csvg width=\\'52\\' height=\\'26\\' viewBox=\\'0 0 52 26\\' xmlns=\\'http://www.w3.org/2000/svg\\'%3E%3Cg fill=\\'none\\' fill-rule=\\'evenodd\\'%3E%3Cg fill=\\'%23ff8abc\\' fill-opacity=\\'0.1\\'%3E%3Cpath d=\\'M10 10c0-2.21-1.79-4-4-4-3.314 0-6-2.686-6-6h2c0 2.21 1.79 4 4 4 3.314 0 6 2.686 6 6 0 2.21 1.79 4 4 4 3.314 0 6 2.686 6 6 0 2.21 1.79 4 4 4v2c-3.314 0-6-2.686-6-6 0-2.21-1.79-4-4-4-3.314 0-6-2.686-6-6zm25.464-1.95l8.486 8.486-1.414 1.414-8.486-8.486 1.414-1.414z\\' /%3E%3C/g%3E%3C/g%3E%3C/svg%3E')]">
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-8 animate-scale-in">
                <h1 className="text-3xl md:text-4xl font-bold text-gradient">
                  Daily <span className="text-candy-bright-pink">Tournaments</span>
                </h1>
              ...
              <div className="flex justify-center mb-8">
                <div className="bg-candy-purple/20 rounded-full p-1 flex animate-fade-in">
                  {(['live', 'upcoming', 'completed'] as const).map((tab) => (
                    <button
                      ...
                      className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${
                        activeTab === tab 
                          ? 'bg-candy-bright-pink text-white' 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-scale-in">
                {filteredTournaments.map((tournament) => (
                  <div key={tournament.id} className="candy-panel p-6 rounded-xl hover:scale-105 transition-transform duration-300">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex items-center">
                        <div className="w-12 h-12 flex items-center justify-center rounded-full bg-candy-purple/30 text-2xl animate-pulse-soft">
                      ...
                      {tournament.status === 'live' && (
                        <span className="bg-candy-red/20 text-candy-red text-xs px-2 py-1 rounded-full animate-pulse">
                          Live
                        </span>
                      )}
                    </div>
                    ...
                    <div className="space-y-3 mb-4">
                      <div className="flex items-center text-white/80 text-sm">
                        <Users className="w-4 h-4 mr-2 text-candy-yellow" />
                        <span>{tournament.participants}/{tournament.maxParticipants} Participants</span>
                      </div>
                      
                      <div className="flex items-center text-white/80 text-sm">
                        <Clock className="w-4 h-4 mr-2 text-candy-yellow" />
                        <span>{tournament.startTime}</span>
                      </div>
                      
                      <div className="flex items-center text-white/80 text-sm">
                        <Trophy className="w-4 h-4 mr-2 text-candy-yellow" />
                    ...
                    {tournament.status === 'live' && (
                      <Button 
                        className="candy-button w-full animate-pulse-soft"
                    ...
                    {tournament.status === 'upcoming' && (
                      <Button 
                        className="candy-accent-button w-full"
              ...
              {/* Featured Tournament */}
              {activeTab === 'live' && (
                <div className="mt-12 candy-panel p-8 rounded-xl animate-fade-in hover:border-candy-yellow transition-colors">
                  <div className="flex flex-col md:flex-row justify-between">
                    <div>
                      <div className="flex items-center mb-4">
                        <div className="w-16 h-16 flex items-center justify-center rounded-full bg-candy-yellow/20 text-4xl animate-pulse-soft">
                          ♟️
                        </div>
                        <div className="ml-4">
                          <h2 className="text-2xl font-bold text-white">Weekend Chess Tournament</h2>
                          <div className="flex items-center mt-1">
                            <span className="bg-candy-red/20 text-candy-red text-xs px-2 py-1 rounded-full mr-2">
                              Featured
                            </span>
                      ...
                      <p className="text-white/80 mb-6 max-w-2xl">
                        ...
                        <span className="text-candy-red italic block mt-2">
                          "Our AI will roast your every move, but don't take it personally - it roasts everyone equally badly!"
                        </span>
                      </p>
                      
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        <div className="candy-item p-3 hover:bg-white/95 transition-colors">
                          <p className="text-candy-bright-pink text-sm font-medium">Grand Prize</p>
                          <p className="text-foreground">1000 Points + Trophy</p>
                        </div>
                        <div className="candy-item p-3 hover:bg-white/95 transition-colors">
                          <p className="text-candy-bright-pink text-sm font-medium">Format</p>
                          <p className="text-foreground">Swiss System</p>
                        </div>
                        <div className="candy-item p-3 hover:bg-white/95 transition-colors">
                          <p className="text-candy-bright-pink text-sm font-medium">Duration</p>
                    ...
                    <div className="mt-6 md:mt-0 md:ml-6 flex flex-col justify-center items-center md:items-end">
                      <Button 
                        className="candy-button w-full md:w-auto px-8 py-6 text-lg animate-pulse-soft"
